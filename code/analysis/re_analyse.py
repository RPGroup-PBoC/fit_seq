...
folder_list = glob.glob('../processing/growth_curves_plate_reader/*')
...
for file in folder_list:
    if not os.path.isdir(file+'/new_output/'):
        os.mkdir(file+'/new_output/')
    df_path = glob.glob(file + '/output/*_growth_plate.csv')[0]
    ...
    if 'blank_val' in data.columns:
        data['OD_sub'] = data['OD600'] - data['blank_val']
    else:
        data['OD_sub'] = data['OD600']
    ...
    if (not REPLOT):
        ...
        # Export result
        df_gp.to_csv(file + '/new_output/gp_per_well.csv',
                     index=False)
    """
    # Read derivatives
    df_gp = pd.read_csv(f'output/*_gp_per_well.csv')
    ...
    with PdfPages('output/growth_rate_per_well.pdf') as pdf:
        ...
        layout = pd.read_excel(file + '/*_plate_layout.xlsx', sheet_name='well',
                                     header=None).values
